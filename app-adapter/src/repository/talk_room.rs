...
impl TalkRoomRepository for DbFirestoreRepositoryImpl<TalkRoom> {
    async fn get_talk_room(&self, primary_user_id: PrimaryUserId) -> anyhow::Result<TalkRoom> {
        ...
        Ok(TalkRoomWrapper::from((
            talk_room_table.clone(),
            talk_room_card_table,
...
"#,
        ...
        firestore
            ...
            .map_err(|e| {
                ...
                anyhow!(RepositoryError::CouldNotInsert(
                    ...
                    talk_room_table.document_id.clone(),
                ))
            })?;
        firestore
            .fluent()
            ...
            .map_err(|e| {
                ...
                anyhow!(RepositoryError::CouldNotInsert(
                    TALK_ROOM_CARD_COLLECTION_NAME.to_string(),
                    "document_id".to_string(),
                    talk_room_card_table.document_id.clone(),
                ))
            })?;
         ...
         * イベントを作成する
         */
        let event = self.create_event(source.clone()).await?;

        Ok(TalkRoomWrapper::from((talk_room_table.clone(), talk_room_card_table, event)).0)
    }
