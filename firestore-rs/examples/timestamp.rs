...
struct MyTestStructure {
    ...
    updated_at_attr: Option<DateTime<Local>>,

    #[serde(default)]
    #[serde(with = "firestore::serialize_as_optional_timestamp")]
    updated_at_attr_always_none: Option<DateTime<Local>>,
}
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let my_struct = MyTestStructure {
        ...
        updated_at: Some(Local::now().into()),
        updated_at_always_none: None,
        updated_at_attr: Some(Local::now().into()),
    ...
    let objects1: Vec<MyTestStructure> = db
        ...
        .filter(|q| {
            q.for_all([q
                ...
                .less_than_or_equal(
                    firestore::FirestoreTimestamp(Local::now()), // Using the wrapping type to indicate serialization without attribute
                )])
        })
        .obj()
        .query()
