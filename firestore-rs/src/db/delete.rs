...
impl FirestoreDeleteSupport for FirestoreDb {
        ...
        S: AsRef<str> + Send,
    {
        ...
        let begin_query_local: DateTime<Local> = Local::now();
        self.client().get().delete_document(request).await?;
        let end_query_local: DateTime<Local> = Local::now();
        let query_duration = end_query_local.signed_duration_since(begin_query_local);
