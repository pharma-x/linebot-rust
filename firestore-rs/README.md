...
## Timestamps support
By default, the types such as DateTime<Local> serializes as a string
...
```rust
   q.field(path!(MyTestStructure::created_at))
     .less_than_or_equal(firestore::FirestoreTimestamp(Local::now()))
```
...
let object_returned = db
    ...
    .document(FirestoreDb::serialize_map_to_doc("",
      [
        ...
        ("created_at", FirestoreTimestamp(Local::now()).into()),
      ]
     )?)
    .execute()
    .await?;
...
test_null: Option<DateTime<Local>>,
```
